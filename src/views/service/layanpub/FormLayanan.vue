  ...
  <div class="min-h-screen bg-gray-50 p-4 md:p-8 relative">
    <div class="md:max-w-[92vw] mx-auto grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <div class="md:col-span-2 bg-white rounded-2xl shadow p-6">
        ...
        <div class="text-gray-700 space-y-10">
          <div
          ...
          >
            ...
            />
            <div class="flex-1">
              <h3 class="text-slate-900 font-semibold mb-2">{{ section.title }}</h3>
              <div
                v-html="section.html"
...
// State
const user         = ref({ name: '', email: '', no_hp: '' })
const sections     = ref([])
const note         = ref('')
const fileList     = ref([])
const toasts       = ref([])
...
async function fetchSK() {
  try {
    ...
    const sk  = res.data.data[0] || {}
    sections.value = [
      { key: 'persyaratan', icon: CheckCircleIcon, title: 'Persyaratan',         html: sk.persyaratan },
      { key: 'simpro',      icon: DocumentTextIcon, title: 'Sistem & Prosedur',  html: sk.simpro },
      { key: 'durasi',      icon: ClockIcon,         title: 'Jangka Waktu',       html: `<p>${sk.durasi}</p>` },
      { key: 'biaya',       icon: BanknotesIcon,     title: 'Biaya/Tarif',        html: `<p>Rp ${sk.biaya}</p>` },
      { key: 'produk',      icon: TagIcon,           title: 'Produk Layanan',     html: `<p>${sk.produk_layanan}</p>` },
      { key: 'penanganan',  icon: ArrowUpOnSquareIcon, title: 'Penanganan & Masukan', html: sk.penanganan }
    ]
  } catch {
