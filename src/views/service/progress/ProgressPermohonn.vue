<template>
  <div class="w-full flex flex-col justify-center px-4 md:px-10 bg-gray-50">
    ...
    <div class="bg-white rounded-xl shadow-lg p-4 md:p-6 space-y-6 mb-16 md:mb-72">
        ...
        <table class="min-w-[600px] w-full bg-white rounded-lg shadow-sm overflow-hidden">
          <thead>
            <tr class="bg-gradient-to-r from-green-600 to-blue-600">
              <th class="px-4 py-2 md:px-6 md:py-3 text-left text-xs md:text-sm font-semibold text-white uppercase tracking-wide">No</th>
              <th class="px-4 py-2 md:px-6 md:py-3 text-left text-xs md:text-sm font-semibold text-white uppercase tracking-wide">Layanan</th>
              <th class="px-4 py-2 md:px-6 md:py-3 text-left text-xs md:text-sm font-semibold text-white uppercase tracking-wide">Pengguna</th>
              <th class="px-4 py-2 md:px-6 md:py-3 text-left text-xs md:text-sm font-semibold text-white uppercase tracking-wide">Status</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-200">
            ...
            >
              <td class="px-4 py-2 md:px-6 md:py-4 whitespace-nowrap text-xs md:text-sm text-gray-700">
                {{ item.noPermohonan }}
              </td>
              <td class="px-4 py-2 md:px-6 md:py-4 whitespace-nowrap text-xs md:text-sm text-gray-700">
                {{ item.layanan }}
              ...
              <td class="px-4 py-2 md:px-6 md:py-4 whitespace-nowrap">
                <span
                  :class="[
                    ...
                    item.status.toLowerCase() === 'disetujui' ? 'bg-green-100 text-green-800' : '',
                    item.status.toLowerCase() === 'diproses'  ? 'bg-yellow-100 text-yellow-800' : '',
                    item.status.toLowerCase() === 'ditolak'   ? 'bg-red-100 text-red-800' : ''
                  ]"
                >
                  {{ item.status }}
                </span>
      ...
      <!-- Pagination -->
      <div class="flex flex-row justify-center items-center space-y-2 sm:space-y-0 sm:space-x-12">
        <button
          @click="prevPage"
        ...
        <span class="text-gray-700 font-medium text-sm mr-4">{{ currentPage }} dari {{ totalPages }}</span>
        <button
          @click="nextPage"
...
const layananOptions = computed(() =>
  [...new Set(requests.value.map(r => r.layanan))]
)
